...
describe('Twitter Proxy', function(){
  ...
  describe('Failure', function(){
     ...
     describe('statusCode !== 200', function(){
       ...
       it('all other phases - pipe back to client', function(done){ // test error in all other phases
           ...
           tp.createTwtRequest(options, function(){
             ...
             this.response.on('end', function(){   // note this is response is pair of client request (mocked)
                                                   ...
                                                   // calling 'end' event handler indicates succesfull pipe
             })
       ...
       it('response error - call next(..)', function(done){        // test error in all other phases
           ...
           tp.createTwtRequest(options, function(){
             ...
             this.twtResponse.emit('error', errorStr); // error is emmited so twtResponseOnFailure() doesn't run  
       ...
       it('response error - Promise aware', function(done){        // test error in all other phases
           var reject;
           ...
           p.then(null,
               function rejected(err){
                  assert.equal(err, errorStr); // check that we have same error a as in next()
                  done();   
               }
           )
           tp.next = function(err){                // next will be called in response error handler
           ...
           tp.createTwtRequest(options, function(){
             this.twtResponseOnError(reject);          // calls next(..) and also rejects promise
